 ...
 */
#[AsCommand(
    name: 'astryx-typo3:site:seed',
    description: 'Create or update the Astryx showcase site page tree.'
)]
final class SeedAstryxSiteCommand extends Command
{
    ...
    private const FAL_FOLDER = 'astryx-typo3-site';
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addOption('include-video', null, InputOption::VALUE_NONE, 'Include video/demo elements in seeded showcase content. Disabled by default.')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $chapters = AstryxSiteDefinitions::chapters();
        $supportPages = AstryxSiteDefinitions::supportPages();

        if ($dryRun) {
            $io->section('Would seed');
            $io->listing([
                sprintf('Site root "%s" below page %d', AstryxSiteDefinitions::ROOT_TITLE, $parentPid),
        ...
        $rootAttributes = [
            'is_siteroot' => 1,
            'backend_layout' => AstryxSiteDefinitions::LAYOUT_STARTPAGE,
            'backend_layout_next_level' => AstryxSiteDefinitions::LAYOUT_CONTENTPAGE,
            'abstract' => 'The Astryx design system, server-rendered for TYPO3.',
        ];
        $existingRootUid = $this->findRootPageUid($parentPid, AstryxSiteDefinitions::ROOT_TITLE);
        if ($existingRootUid !== null) {
            $upserter->update(
                $existingRootUid,
                AstryxSiteDefinitions::ROOT_TITLE,
                AstryxSiteDefinitions::ROOT_SLUG,
                self::SORTING_STEP,
                $now,
                $pageColumns,
                $rootAttributes,
        ...
        foreach ($chapters as $chapter) {
            ...
            $uid = $this->upsertPage(
                ...
                $pageColumns,
                [
                    'backend_layout' => AstryxSiteDefinitions::LAYOUT_CONTENTPAGE,
                    ...
                    'tx_desiderioastryx_theme' => $chapter['theme'],
                ],
            );
        ...
        if ($themesUid > 0) {
            ...
            foreach (AstryxSiteDefinitions::themes() as $theme) {
        ...
        $io->section('Next: write config/sites/astryx-typo3/');
        $io->listing([
            sprintf('rootPageId: %d', $rootUid),
            sprintf('errorHandling 404 target: t3://page?uid=%d', $errorUid),
            sprintf('astryx.footer.legalPageIds: \'%s\'', implode(',', $legalUids)),
            sprintf('astryx.search.targetPageId: \'%d\' (and search.enabled: true)', $searchUid),
            'The search page needs the webconsulting/astryx-typo3-search set '
     ...
     */
    private function seedChapterContent(SymfonyStyle $io, array $pages, array $chapters, int $now, bool $includeVideo): void
    {
        $manifestPath = GeneralUtility::getFileAbsFileName(
            'EXT:astryx_typo3/Resources/Private/Data/astryx-content-groups.json',
        );
        ...
        foreach ($chapters as $chapter) {
            ...
            foreach ($elements as $element) {
                $cType = $element['ctype'];
                if (!$includeVideo && $this->isVideoElement((string)$cType)) {
                    continue;
                }
    ...
    private function isVideoElement(string $cType): bool
    {
        return str_contains($cType, 'video') || $cType === 'astryx_typo3_featuredemo';
    }
    ...
    private function seedHomeContent(SymfonyStyle $io, int $rootUid, int $hubUid, int $themesUid, int $now): void
    {
        ...
        $sorting = 0;
        $placed = 0;
        foreach (AstryxSiteDefinitions::homeContent() as $block) {
     ...
     *
     * One Rich Text element per page, from AstryxSiteDefinitions::supportContent().
     ...
     * `astryx.search.targetPageId` something an editor can point at.
     */
    private function seedSearchContent(SymfonyStyle $io, int $searchUid, int $now): void
    {
        ...
        if ($record === null) {
            ...
            $seeder->insert($searchUid, $now, $resolver->buildContentInsert(
                ...
                [
                    ...
                    'lead' => AstryxSiteDefinitions::SEARCH_LEAD,
                    'width' => 'lg',
                    'align' => 'start',
                ],
                self::SORTING_STEP,
        ...
        $row = [
            'pid' => $searchUid,
            'CType' => AstryxSiteDefinitions::SEARCH_PLUGIN_CTYPE,
            'colPos' => 0,
     ...
     * AstryxSiteDefinitions::chapters() appears here without anyone editing a
     * fixture.
     *
    ...
    private function seedHubContent(SymfonyStyle $io, int $hubUid, array $pages, array $chapters, int $now): void
    {
        ...
        $cType = 'astryx_typo3_menucardgrid';
        $record = $catalog[$cType] ?? null;
        if ($record === null) {
